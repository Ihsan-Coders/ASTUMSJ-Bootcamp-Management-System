...
export default function AssignmentForm({ batchId, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await createAssignment({ ...form, batch: batchId, maxScore: Number(form.maxScore) });
      setForm({ title: '', description: '', instructions: '', deadline: '', maxScore: 100 });
      onCreated?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-card glow-border rounded-lg p-6 space-y-3">
      <h3 className="text-lg font-semibold text-text-primary">Create Assignment</h3>
      ...
      <textarea name="description" placeholder="Description" value={form.description} onChange={handleChange} required
        className="w-full p-2 rounded border border-border bg-background text-text-primary" />
      <textarea name="instructions" placeholder="Instructions (optional)" value={form.instructions} onChange={handleChange}
        className="w-full p-2 rounded border border-border bg-background text-text-primary" />
      <div className="grid grid-cols-2 gap-3">
        <input type="date" name="deadline" value={form.deadline} onChange={handleChange} required
          className="w-full p-2 rounded border border-border bg-background text-text-primary" />
        <input type="number" name="maxScore" placeholder="Max Score" value={form.maxScore} onChange={handleChange} min="1" required
          className="w-full p-2 rounded border border-border bg-background text-text-primary" />
      </div>
